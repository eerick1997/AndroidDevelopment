...
class TrackingFragment : Fragment(), MenuProvider {
    ...
    private fun endRunAndSaveToDb() {
        map?.snapshot { bitmap ->
            ...
            val averageSpeed = round((distanceInMeters / 1000f) / (currentTimeMillis / 1000f / 60 / 60) * 10) / 10f
            val dateTimestamp = Calendar.getInstance().timeInMillis
            val caloriesBurned = ((distanceInMeters / 1000f) * weight).toInt()
            val run = Run(bitmap, dateTimestamp, averageSpeed, distanceInMeters, currentTimeMillis, caloriesBurned)
